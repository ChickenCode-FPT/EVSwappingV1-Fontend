// src\app\features\station\services\geolocation.service.ts
...
export class GeolocationService {
  ...
  getCurrentLocation(
    ...
    return new Promise((resolve, reject) => {
      if (!navigator.geolocation) {
        ...
        statusCb?.('Trình duyệt không hỗ trợ geolocation');
        return reject(err);
      }
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            ...
            if (attempt < retries) {
              ...
              if (useHighAccuracy) {
                statusCb?.('HighAccuracy fail sau nhiều lần → thử lại với LowAcc');
                console.warn('HighAccuracy fail sau nhiều lần → fallback sang LowAcc');
  ...
  watchLocation(callback: (coords: [number, number]) => void, useHighAccuracy: boolean = true) {
    if (!navigator.geolocation) {
      console.warn('Geolocation không hỗ trợ trên browser này');
      return;
    }
