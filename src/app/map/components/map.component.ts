...
export class MapComponent implements AfterViewInit {
  ...
  private addStationMarkers() {
    this.stations.forEach((station, index) => {
      ...
      const m = this.mapSvc.createMarker(
        this.mapboxgl,
        this.mapSvc.colors.station,
        station.coords
      );
  ...
  private async findNearestStationAndRoute() {
    ...
    try {
      ...
      const nearestStation = await firstValueFrom(
        this.stationApi.getNearestStation(lng, lat)
      );
      if (!nearestStation) {
  ...
  private async drawRouteToStation(
    stationId: number,
    lng: number,
    lat: number
  ) {
    try {
      const route = await firstValueFrom(
        this.stationApi.getRouteToStation(stationId, lng, lat)
      );
  ...
  private updateRouteInfoControl() {
    ...
    const btn = (ctrlDiv as HTMLElement).querySelector(
      '#ctrl-btn'
